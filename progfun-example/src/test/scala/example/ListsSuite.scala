...
class ListsSuite extends FunSuite {
  ...
  test("sum of a negative numbers") {
    assert(sum(List(-10,-2,-8)) === -20)
  }
  
  test("sum of positive and negative numbers") {
    assert(sum(List(-1,1,5-3,2)) === 4)
  }
  ...
  test("max of equals numbers") {
    assert(max(List(0, 0, 0)) === 0)
  }
  
  test("max of one member list") {
    assert(max(List(5)) === 5)
  }
  
  test("max of negative numbers") {
    assert(max(List(-5, -1, -3, -7, -10)) === -1)
  }
  
  test("max of repeated numbers") {
    assert(max(List(-6, 3, 1, -2, 3, 0)) === 3)
  }
